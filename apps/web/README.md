...
- `NEXT_PUBLIC_API_BASE_URL` (optional client override; defaults to `http://localhost:3001` in development and `https://api.hiptrip.net` in production)
- `NEXT_PUBLIC_TRIP_PLAN_SOURCE_BADGE` (`dev` | `always` | `never`; defaults to `dev`)
- `API_BASE_URL` (recommended in production for server proxy routes like `/suggestions`; example: `https://api.hiptrip.net`)
